...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'amoskipkirui0@gmail.com', 
                      [cd['to']])
            sent = True
...
#         if form.is_valid():
#             cd = form.cleaned_data
#             scheme = request.META.get('HTTP_X_FORWARDED_PROTO') or request.META.get('wsgi.url_scheme')
#             netloc = request.get_host()
#             post_url = f"{scheme}://{netloc}{post.get_absolute_url()}"
#             subject = f"{cd['name']} recommends you read" \
#                       f"{post.title}"
#             message = f"Read {post.title} at {post_url}\n\n" \
#                       f"{cd['name']}\'s comments: {cd['comments']}"
